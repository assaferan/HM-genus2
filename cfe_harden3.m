...
LOGF := "cfe_harden3_out.txt"; PrintFile(LOGF, "# cfe_final2" : Overwrite := true);
...
for p in PrimesUpTo(300000) do
    if p eq 2 then continue; end if;
    for pe in Factorization(p*OK) do
        P := pe[1]; fP := Degree(ResidueClassField(P)); q := Norm(P);
        if q gt 300000 then continue; end if;   // Norm > cutoff => contributes nothing below cutoff
...
for c in [6..12] do
    N := 5^4 * 66179 * 4^c;
    L := LSeries(2, [0,0,0,0,1,1,1,1], N, cf : Sign := 0, Precision := 3);
