// grh_881.m -- GRH-conditional certificate that sigma = rho_f for the l=13 torsion example
// (curve 881.1 over F=Q(sqrt2)).
//
// sigma = the 2-dim irreducible sub of A[13] (A[13]^ss = 1 + chi_13 + sigma); rho_f = the
// mod-lambda reduction of the Hilbert newform f at level p2^3*(21a+1) (norm 7048), wt [2,2],
// Hecke field: deg-18, Galois S_18 (13 splits variously; take a lambda|13 whose reduction
// gives sigma). Both are 2-dim, irreducible, det = chi_13, unramified outside S = {P_881, 2, 13}.
...
// the splitting field) makes this finite. cond(sigma) ~ 7048*13^a, so (log cond)^2 ~ 200;
// we verify EXACT trace agreement for all good primes of F up to BOUND >> that.
//
//   tr sigma(Frob_P) = a_P(A) - 1 - N(P) = -#C(F_P)   (mod 13)
//   tr rho_f(Frob_P) = a_P(f) mod lambda  (in F_13)
//
// Run: magma grh_881.m
SetColumns(0);
K<a> := QuadraticField(2); OK := Integers(K); R<x> := PolynomialRing(K);
l := 13; Fl := GF(l); RT := PolynomialRing(Fl);
BOUND := 3000;
LOGF := "grh_881_out.txt";
PrintFile(LOGF, "# grh_881: GRH-conditional trace check sigma = rho_f, l=13, curve 881.1" : Overwrite := true);
procedure LOG(s) printf "%o\n", s; PrintFile(LOGF, s); end procedure;

// curve 881.1
f := a*x^6 + (1+2*a)*x^5 + 2*a*x^4 + (-2+a)*x^3 - a*x^2 + (-1+a)*x + 1;
hh := (1-a)*x^3 - a*x^2 + (1-a)*x;
C := HyperellipticCurve(f, hh);
p2 := Factorization(2*OK)[1][1];
Ncond := ideal<OK | 21*a + 1> * p2^3;             // norm 7048 = 2^3 * 881 (e=3)

// ---- build the matching eigenform, pick lambda | 11 ----
LOG("building eigenform at level norm 7048 ...");
t0 := Cputime();
...
for nf in decomp do
    ...
    for pr in Factorization(l*OF) do
        kf0, m0 := ResidueClassField(pr[1]);                       // any residue degree; match check ensures F_13 traces
        good := true;
...
LOG(Sprintf("  eigenform ready (%.1o s); Hecke field deg %o, 13 %o", Cputime(t0),
    Degree(OFF), Sprintf("lambda residue deg %o", Degree(kf))));

// ---- sigma irreducible? char poly of sigma at some prime irreducible over F_13 ----
irr := false;
for q in fp do
    cp := RT ! (RT.1^2 - (Integers()!q[2])*RT.1 + (Norm(q[1]) mod l));
    if IsIrreducible(cp) then irr := true; break; end if;
end for;
LOG(Sprintf("  sigma irreducible (a char poly is irreducible over F_13): %o", irr));
...
if #fails eq 0 and irr then
    ...
    LOG("    sigma, rho_f: 2-dim, irreducible, det = chi_13, unramified outside {P_881, 2, 13}.");
    LOG("    cond(sigma) ~ 7048*13^a, (log cond)^2 ~ 200; the GRH conductor-based effective");
