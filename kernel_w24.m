...
lev := J;
dd := 1;   // normalization power for [2,4]
...
t0 := Cputime();
M := HilbertCuspForms(F, lev, [2,4]); dm := Dimension(M);
printf "idx %o: level norm %o, [2,4] dim %o (built %.1os)\n", idx, Norm(lev), dm, Cputime(t0);
...
printf ">>> idx %o [2,4] kernel: %o (true %o / ctrl %o)  [%.1os]\n",
    idx, status, Dimension(cur), Dimension(cur2), Cputime(t0);
exit;
