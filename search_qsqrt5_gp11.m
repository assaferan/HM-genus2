// search_qsqrt5_gp11.m -- Screening loop for the (1,11) pipeline over K = Q(sqrt(5)).
//
// Direct analogue of search_quadratic_gp11.m for K = Q(sqrt(2)), but over Q(sqrt(5)).
// Note: 11 SPLITS in Q(sqrt(5)) (since (5/11) = 1 by QR), so there are two primes
// above 11, each of norm 11. The potential-good-at-11 filter checks both.
//
// Enumerates non-rational K-points (x1:x2:x3:x4) in P^3(K) with bounded height,
// computes the Pfaffian polynomial g(t) = Pf(S)(x1,x2,x3,x4,t) in K[t], finds
// K-rational roots t0, and for each D_2 point inverts to a genus-2 curve.
//
// Filters:
//   - non-degenerate D_2: Rank(S) = 4
//   - inversion convergence
//   - genuinely over K: Igusa invariants not all rational
//   - potential good reduction at 11
//   - geometric End = Z (simple abelian surface)
// Survivors stream to survivors_qsqrt5_gp11.m; run analyze_qsqrt5_gp11.m.
//
// Run: magma search_qsqrt5_gp11.m
...
PrintFile(survfile, "k<s5> := QuadraticField(5); survivors_qsqrt5_gp11 := [* *];");
...
printf "=== %o survivors recorded to %o; run analyze_qsqrt5_gp11.m ===\n", nsurv, survfile;
